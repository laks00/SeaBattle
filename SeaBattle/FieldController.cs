...
namespace SeaBattle
{
    public class FieldController
    {
        ...
        {
            const int indent = 120;
            for (int y = 0; y < FieldSize; y++)
            {
                for (int x = 0; x < FieldSize; x++)
                {
                    int xIndent = indent + markingOffset - 64;
        ...
        private void AddMarking(int coordinate, int offset)
        {
            ...
            {
                Location = new Point(doubleButtonSize + offset + coordinateOffset +4, buttonSize + 64),
                Text = Convert.ToString(EnglishLetters[coordinate]),
                AutoSize = true
            };
            Label digit = new Label
            {
                Location = new Point(buttonSize + offset +8, doubleButtonSize  + coordinateOffset +68),
